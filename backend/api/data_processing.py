...
class DataProcessor:
    def __init__(self):
        self.feature_columns = [
            ...
            'eq_temp', 'stellar_radius', 'stellar_mass'
        ]
    ...
    def _generate_synthetic_data(self, n_samples=5000, source='kepler'):
        ...
        data = {
            'orbital_period': np.random.lognormal(1.5, 1.5, n_samples),  # days
            'transit_duration': np.random.gamma(2, 2, n_samples),  # hours
            'planetary_radius': np.random.lognormal(0, 0.8, n_samples),  # Earth radii
            'eq_temp': np.random.normal(500, 300, n_samples),  # Kelvin
            'stellar_radius': np.random.lognormal(0, 0.3, n_samples),  # Solar radii
            'stellar_mass': np.random.lognormal(0, 0.3, n_samples)  # Solar masses
        }
        ...
        for _, row in df.iterrows():
            ...
            if (1 < row['orbital_period'] < 100 and 
                1 < row['transit_duration'] < 10 and
                0.5 < row['planetary_radius'] < 10 and
                200 < row['eq_temp'] < 1000):
                label = 2  # Confirmed
            # Candidate: some unusual but possible characteristics
            elif (0.5 < row['orbital_period'] < 500 and
                  0.1 < row['planetary_radius'] < 20):
                label = 1  # Candidate
